...
let selectedPlats   = ['reddit', 'twitter', 'facebook', 'google'];
...
function togglePlat(btn) {
  ...
  if (selectedPlats.includes(p)) {
    if (selectedPlats.length === 1) return; // keep at least one
...
function resetUI(brand, region) {
  ...
  [0, 1, 2, 3, 4, 5, 6].forEach(i => setIcon(i, ''));
}
...
async function runAgents() {
  ...
  try {
    ...
    // Agent 1 — always runs
    const scoutMentions = await agentScout(brand, competitors, region, allSelected);
    allMentions.push(...scoutMentions);

    // Agents 2–5 — conditional on platform selection
    if (selectedPlats.includes('reddit')) {
      allMentions.push(...await agentReddit(brand, competitors, region, allSelected));
    } else { setIcon(1, ''); }

    if (selectedPlats.includes('twitter')) {
      allMentions.push(...await agentTwitter(brand, competitors, region, allSelected));
    } else { setIcon(2, ''); }
